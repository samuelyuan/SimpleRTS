...
public class GameUnit 
{
	 ...
	 * 
	 * Type3 > Type1
	 * The heavy unit's armor reduces light unit's attack and counterattacks with strong weapons/
	 */
	...
	public Point getCurrentPoint() { return this.currentPosition; }
	...
	public boolean getIsPlayerUnit() { return isPlayerUnit; }
	...
	public boolean isPathCreated() { return pathUnit.getIsPathCreated(); }
	public void startMoving() { pathUnit.startMoving(); }
	...
	public void spawn(int[][] map, Point mapPos, int factionId)
	{
		...
		//make sure to adjust values (+1 for ally since code is 2, +4 for enemy, since code is 5)
	...
	/*
	 * Use AStar pathfinding algorithm to find an optimal path that takes obstacles into account
	 */
	...
	/*public void drawPathfinding()
	{
		if (pathUnit.getIsPathCreated() == true)
		{
			...
			for (int i = pathUnit.getPath().size() - 1; i >= 0; i--)
			{
				...
				SimpleRTS.offscr.drawRect(pathUnit.getPath().get(i).getX() * GameMap.TILE_WIDTH - SimpleRTS.cameraX, 
	...
	/*
	 * When the ally unit is close enough to see the enemy, the ally and enemy will fight each other
	 */
	...
	//(ex. a light unit would do more damage to a medium unit, but less to a heavy unit)
	public int dealDamagePoints(GameUnit enemy)
	{
		if (classType == UNIT_ID_LIGHT)
		{
			if (enemy.classType == UNIT_ID_LIGHT) { return 2; }
			if (enemy.classType == UNIT_ID_MEDIUM) { return 3; }
			if (enemy.classType == UNIT_ID_HEAVY) { return 2; }
		}
		else if (classType == UNIT_ID_MEDIUM)
		{
			if (enemy.classType == UNIT_ID_LIGHT) { return 2; }
			if (enemy.classType == UNIT_ID_MEDIUM) { return 2; }
			if (enemy.classType== UNIT_ID_HEAVY) { return 3; }
		}
		else if (classType == UNIT_ID_HEAVY)
		{
			if (enemy.classType == UNIT_ID_LIGHT) { return 5; }
			if (enemy.classType == UNIT_ID_MEDIUM) { return 2; }
			if (enemy.classType == UNIT_ID_HEAVY) { return 2; }
		}
	...
	/* 
	 * Goal: Make sure enemy is within sight of player so that the player cannot shoot through walls.
	 */
	public boolean checkVisible(int map[][], GameUnit enemy)
	{
		...
		//otherwise, trace a line of sight between the two tiles and determine whether the line intersects any tiles
